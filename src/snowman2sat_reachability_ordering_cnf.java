/**
 * @class snowman2sat_reachability_ordering_cnf
 * @author Miquel Bofill
 * @version 1.0
 * @date 2023-01-12
 * @brief Translation of "A good snowman is hard to build" instance to CNF, with reachability (without move actions), ensuring acyclicity with a strict partial order
*/
...
public class snowman2sat_reachability_ordering_cnf {
    ...
    private static void reachabilityConstraints(int nSteps, Set<Integer> validLocations, Map<Integer,List<Integer>> validNeighbours, DIMACSFormula f) {
		List<String> ls = new LinkedList<>();

		// Collect valid neighbours
		for (Integer p : validLocations) {
			List<Integer> p_validNeighbours = validNeighbours.get(p);
			...
			for (int i = 0; i < nSteps; ++i) {
				...
				ls.add("!r_" + p + "_" + i);
				ls.add("#c_" + p + "_" + i);
				if (!p_validNeighbours.isEmpty())
					for (Integer q : p_validNeighbours)
						ls.add("e_" + q + "_" + p + "_" + i);
				f.addClause(ls);

				// Balls unreachable
				add2DIMACSFormula(implication2cnf(List.of("#bs_" + p + "_" + i, "#bm_" + p + "_" + i, "#bl_" + p + "_" + i), List.of("!r_" + p + "_" + i)), f);

				// Locations in the path are reachable and topologically ordered
				for (Integer q : p_validNeighbours)
					add2DIMACSFormula(implication2cnf(List.of("e_" + p + "_" + q + "_" + i), List.of("r_" + p + "_" + i, "a_" + p + "_" + q + "_" + i)), f);

				// Variables  a_{p,q}^i  define a strict partial order
				f.addClause(List.of("!a_" + p + "_" + p + "_" + i));
				for (Integer q : p_validNeighbours)
					for (Integer r : validLocations)
						if (!q.equals(r))
							f.addClause(List.of("!a_" + p + "_" + q + "_" + i, "!a_" + q + "_" + r + "_" + i, "a_" + p + "_" + r + "_" + i));
			}
		}
    }
    ...
    private static void frameAxioms(int nSteps, Set<Integer> l, DIMACSFormula f) {
		for (int i = 0; i < nSteps; ++i) {
			...
			for (Integer j : l) {
				...
				f.addClause(List.of("!#c_" + j + t1, "r_" + j + t));
    ...
    private static void translate(InputStream in, PrintStream out, int nSteps) throws Exception {
		...
		reachabilityConstraints(nSteps, l, validNeighbours, f);
